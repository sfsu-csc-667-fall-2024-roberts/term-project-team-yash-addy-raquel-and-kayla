...
export class GameModel {
  ...
  static async makeMove(
    ...
    const playerIndex = game.board_state.players.findIndex(
      (p) => p.id === userId,
    );
    ...
    if (this.checkWinCondition(game.board_state.chips, player.color)) {
      ...
      game.board_state.currentPlayerIndex =
        (game.board_state.currentPlayerIndex + 1) %
        game.board_state.players.length;
    }
   ...
   */
  static async listGames(): Promise<
    Array<{
      id: string;
      playerCount: number;
      status: string;
  ...
  private static checkWinCondition(
